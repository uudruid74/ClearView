...
[QUIRKS](https://htmx.org/quirks/) page.
...
Download `htmx.min.js` [from jsDelivr](https://cdn.jsdelivr.net/npm/htmx.org@2.0.10/dist/htmx.min.js) and add it to the appropriate directory in your project
...
type to the given URL when the element is [triggered](https://htmx.org/docs/#triggers):
...
If you want different behavior you can use the [hx-trigger](https://htmx.org/attributes/hx-trigger/)
...
with the [`hx-trigger`](https://htmx.org/attributes/hx-trigger/) attribute:
...
when you are showing the user a [progress bar](https://htmx.org/examples/progress-bar/).
...
If you want the `htmx-request` class added to a different element, you can use the [hx-indicator](https://htmx.org/attributes/hx-indicator/)
...
elements for the duration of a request by using the [hx-disabled-elt](https://htmx.org/attributes/hx-disabled-elt/) attribute.

### [Targets](https://htmx.org/docs/\#targets)

If you want the response to be loaded into a different element other than the one that made the request, you can
use the [hx-target](https://htmx.org/attributes/hx-target/) attribute, which takes a CSS selector. Looking back at our Live Search example:
...
`innerHTML` of the target element. You can modify this by using the [hx-swap](https://htmx.org/attributes/hx-swap/) attribute
...
The [hx-swap](https://htmx.org/attributes/hx-swap/) attribute supports many options for tuning the swapping behavior of htmx. For
...
[hx-swap-oob](https://htmx.org/attributes/hx-swap-oob/) attribute in the _response_ html:
...
If you want to select a subset of the response HTML to swap into the target, you can use the [hx-select](https://htmx.org/attributes/hx-select/)
attribute, which takes a CSS selector and selects the matching elements from the response.

You can also pick out pieces of content for an out-of-band swap by using the [hx-select-oob](https://htmx.org/attributes/hx-select-oob/)
...
even if a swap occurs) you can use the [hx-preserve](https://htmx.org/attributes/hx-preserve/)
...
If you wish to include the values of other elements, you can use the [hx-include](https://htmx.org/attributes/hx-include/) attribute
...
Finally, if you want to programmatically modify the parameters, you can use the [htmx:configRequest](https://htmx.org/events/#htmx:configRequest)
event.

#### [File Upload](https://htmx.org/docs/\#files)

If you wish to upload files via an htmx request, you can set the [hx-encoding](https://htmx.org/attributes/hx-encoding/) attribute to
...
You can include extra values in a request using the [hx-vals](https://htmx.org/attributes/hx-vals/) (name-expression pairs in JSON format) and
[hx-vars](https://htmx.org/attributes/hx-vars/) attributes (comma-separated name-expression pairs that are dynamically computed).

### [Confirming Requests](https://htmx.org/docs/\#confirming)

Often you will want to confirm an action before issuing a request. htmx supports the [`hx-confirm`](https://htmx.org/attributes/hx-confirm/)
...
Using events you can implement more sophisticated confirmation dialogs. The [confirm example](https://htmx.org/examples/confirm/)
...
Another option to do confirmation with is via the [`htmx:confirm` event](https://htmx.org/events/#htmx:confirm). This event
...
with the [`hx-inherit`](https://htmx.org/attributes/hx-inherit/) attribute.

## [Boosting](https://htmx.org/docs/\#boosting)

Htmx supports “boosting” regular HTML anchors and forms with the [hx-boost](https://htmx.org/attributes/hx-boost/) attribute. This
...
Consider the [active search](https://htmx.org/examples/active-search/) example. As it is written, it will not degrade gracefully:
...
the [SSE extension](https://htmx.org/extensions/sse) and [WebSocket extension](https://htmx.org/extensions/ws)
pages to learn more.
...
to the browser’s history, include the [hx-push-url](https://htmx.org/attributes/hx-push-url/) attribute:
...
if you want to use a narrower element for snapshotting you can use the [hx-history-elt](https://htmx.org/attributes/hx-history-elt/)
...
History snapshotting can be disabled for a URL by setting the [hx-history](https://htmx.org/attributes/hx-history/) attribute to `false`
...
document, matched with a [hx-select](https://htmx.org/attributes/hx-select/) tag can be useful too). Htmx will then swap the returned
...
In the event of an error response from the server (e.g. a 404 or a 501), htmx will trigger the [`htmx:responseError`](https://htmx.org/events/#htmx:responseError)
...
Using the [meta config](https://htmx.org/docs/#configuration-options) mechanism for configuring responseHandling, we could add the following
config:
...
Finally, it is worth considering using the [Response Targets](https://htmx.org/extensions/response-targets)
...
the [`hx-validate`](https://htmx.org/attributes/hx-validate/) attribute to “true”.
...
Here is an example of an input that uses the [`hx-on`](https://htmx.org/attributes/hx-on) attribute to catch the
...
Htmx allows you to use [CSS transitions](https://htmx.org/docs/#css_transitions)
...
htmx provides an [extensions](https://htmx.org/extensions) mechanism that allows you to customize the libraries’ behavior.
Extensions [are defined in javascript](https://htmx.org/extensions/building) and then enabled via
the [`hx-ext`](https://htmx.org/attributes/hx-ext/) attribute.
...
The primary integration point between htmx and scripting solutions is the [events](https://htmx.org/docs/#events) that htmx sends and can
respond to. See the SortableJS example in the [3rd Party Javascript](https://htmx.org/docs/#3rd-party) section for a good template for
...
### [Anchor link for: the-hx-on-attributes](https://htmx.org/docs/\#the-hx-on-attributes)[The `hx-on*` Attributes](https://htmx.org/docs/\#hx-on)
...
[Locality of Behaviour (LoB)](https://htmx.org/essays/locality-of-behaviour). Unfortunately, HTML only allows `on*` attributes for a fixed
...
In order to address this shortcoming, htmx offers [`hx-on*`](https://htmx.org/attributes/hx-on) attributes. These attributes allow
...
Another usecase is to [reset user input](https://htmx.org/examples/reset-user-input/) on successful requests using the `afterRequest`
...
Please see the [Web Components Examples](https://htmx.org/examples/web-components/) page for examples on how to integrate htmx
with web components.
...
[Locality of Behavior](https://htmx.org/essays/locality-of-behaviour/), which makes your system easier to understand and
maintain.
...
The first tool htmx provides to help further secure your application is the [`hx-disable`](https://htmx.org/attributes/hx-disable)
...
[`hx-history`](https://htmx.org/attributes/hx-history) attribute anywhere on the page, and setting its value to `false`.
