...
or 4xx, the response from `/register` will replace the contents of the `div` with the `id``any-errors`.
...
_If you are using tools that do not support asterisks in HTML attributes, you_
_may instead use the `x` character, e.g., `hx-target-4xx`._
