...
class Mosaic
{
    ...
    public static function loadMosaic($input): void
    {
        ...
        // '-' that aren't internal prefixes become Lead attributes.
        $sharedVars = null;
        $attrs = [];
        foreach ($input as $key => $value) {
        ...
        if (!empty($attrs)) {
            // Store under Lead inlay as 'attributes' shard
            Shard::loadShard(array_merge(['id' => 'attributes', 'inlay' => 'Lead'], $attrs));
            Exception::debug('VAR', "Lead::attributes set: " . json_encode($attrs));
        }
        if ($sharedVars !== null) {
    ...
    public static function setVar(string $varname, $val, ?string $inlay = null)
    {
        ...
        if ($shard) {
            ...
            if ($varname == 'value') {
                ...
                $data = ['inlay' => $inlay, $field ?? 'value' => $val];
            }
